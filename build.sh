...
prepare() {
    cd $(dirname $0)
    echo "$(grep -v '^$' ./aur-packages.x86_64)" >./aur-packages.x86_64 # 去除文件多余空格
...
genLocalRepository() {
    ...
    cp ./packages.x86_64 ./packages.x86_64.bak #备份配置，以便后续恢复
    mkdir aur
    mkdir ./local-repo
    cd ./aur
    echo "$(cat ../aur-packages.x86_64)" >>../packages.x86_64
    for ((i = 1; i <= $(awk 'END {print NR}' ../aur-packages.x86_64); i++)); do
